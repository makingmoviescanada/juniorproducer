        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {cards.map((card, index) => (
            <ScrollReveal key={index} delay={index * 100}>
              <div 
                className="p-8 bg-junior-parchment border-2 border-junior-black shadow-hard-parchment-sm h-full flex flex-col"
                style={{ borderRadius: "86px" }}
              >
                <h3 className="font-display text-junior-black text-xl uppercase tracking-wider mb-4">
                  {card.title}
                </h3>
                <p className="font-sans text-junior-black/70 italic text-sm mb-4 font-medium">
                  {card.subtitle}
                </p>
                <p className="font-sans text-junior-black/80 text-base leading-relaxed flex-grow">
                  {card.body}
                </p>
              </div>
            </ScrollReveal>
