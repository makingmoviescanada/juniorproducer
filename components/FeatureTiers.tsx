      ...
      {features.map((feature, index) => (
        <div key={index} className="space-y-6">
          <div className="border-b-2 border-junior-red pb-4">
            <h3 className="feature-category font-display text-junior-black text-lg md:text-xl font-black uppercase tracking-wider">
              {feature.category}
            </h3>
          </div>
          ...
          <ul className="space-y-4">
            {feature.items.map((item, itemIndex) => (
              <li key={itemIndex} className="flex items-start gap-4">
                ...
                <span className="feature-item font-sans text-base md:text-lg leading-relaxed md:leading-loose text-junior-black">
                  {item}
                </span>
              </li>
            ))}
